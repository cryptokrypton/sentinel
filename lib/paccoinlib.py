...
def is_valid_paccoin_address(address, network='mainnet'):
    ...
    try:
        ...
        # rescue from exception, not a valid Paccoin address
        return False

    if (address_version != paccoin_version):
        return False
...
# shims 'til we can fix the paccoind side
def SHIM_serialise_for_paccoind(sentinel_hex):
    from models import PACCOIND_GOVOBJ_TYPES
    # unpack
    ...
    # add 'type' attribute
    obj[1]['type'] = PACCOIND_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in paccoind
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # paccoind expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    paccoind_hex = serialise(obj)
    return paccoind_hex


# shims 'til we can fix the paccoind side
def SHIM_deserialise_from_paccoind(paccoind_hex):
    from models import PACCOIND_GOVOBJ_TYPES

    # unpack
    obj = deserialise(paccoind_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('paccoin.conf')
