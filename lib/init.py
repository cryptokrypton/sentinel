...
def has_paccoin_conf():
    import config
    import io

    valid_paccoin_conf = False

    # ensure paccoin_conf exists & readable
    #
    # if not, print a message stating that Paccoin Core must be installed and
    # configured, including JSONRPC access in paccoin.conf
    try:
        f = io.open(config.paccoin_conf)
        valid_paccoin_conf = True
    except IOError as e:
        print(e)

    return valid_paccoin_conf
...
def main():
    ...
    if not has_paccoin_conf():
        print("PaccoinCore must be installed and configured, including JSONRPC access in paccoin.conf")
        sys.exit(1)
