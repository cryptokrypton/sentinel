...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, paccoind):
        golist = paccoind.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_paccoind(paccoind, item)
    ...
    @classmethod
    def import_gobject_from_paccoind(self, paccoind, rec):
        import decimal
        import paccoinlib
        import inflection
        ...
        # shim/paccoind conversion
        object_hex = paccoinlib.SHIM_deserialise_from_paccoind(object_hex)
        objects = paccoinlib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from paccoind...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from paccoind, with every run
        ...
        # get/create, then sync payment amounts, etc. from paccoind - Paccoind is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(paccoind)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from paccoind! %s" % e)
            govobj.vote_delete(paccoind)
            return (govobj, None)
    ...
    def vote_delete(self, paccoind):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(paccoind, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, paccoind, signal, outcome):
        import paccoinlib
        ...
        output = paccoind.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = paccoinlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(paccoind, signal)

    def sync_network_vote(self, paccoind, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = paccoind.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = PACCOIND_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 paccoin addr, non-multisig
            if not paccoinlib.is_valid_paccoin_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Paccoin address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (paccoinlib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/PaccoinDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import paccoinlib
        obj_data = paccoinlib.SHIM_serialise_for_paccoind(self.serialise())
    ...
    def prepare(self, paccoind):
        try:
            object_hash = paccoind.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = PACCOIND_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not paccoinlib.is_valid_paccoin_address(addr, config.network):
    ...
    def hash(self):
        import paccoinlib
        return paccoinlib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = PACCOIND_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, paccoind):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - paccoind.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, paccoind):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - paccoind.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, paccoind):
        now = int(time.time())
        return (self.created_at < (now - paccoind.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, paccoind):
        if self.is_expired(paccoind):
            return False

        return True

    def is_deletable(self, paccoind):
        if self.is_expired(paccoind):
            return True
