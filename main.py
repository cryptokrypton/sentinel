...
from lib import paccoin_config
import time
import datetime
import sys
import os
...
def fix_masternode(data_folder):
    try:
        ...
        if not os.path.isfile(wallet_file):
            print('It seems like the data folder (the one containing wallet.dat) is not the same as the folder where paccoin.conf is')
            data_folder = input('Please, write the data folder path: ')
            return fix_masternode(data_folder)

        print(colored('Make a copy of "wallet.dat" and "paccoin.conf" before continuing.\nThis program will try it best not to touch them, but just in case!', attrs=['bold']))
        ...
        for filename in os.listdir(data_folder):
            if filename in ('wallet.dat', 'paccoin.conf'):
                continue
        ...
        print(colored('Checking paccoin.conf contents', 'green'))
        tokens = paccoin_config.PaccoinConfig.tokenize(config.paccoin_conf)
        ...
        tokens['rpcport'] = tokens.get('rpcport', 7111)
        ...
        try:
            ...
            r = None
            while r not in ('y', 'n'):
                print(colored('It seems like your paccoin.conf is not setting the wallet as masternode!', 'red'))
        ...
        if 'masternodeprivkey' not in tokens:
            print(colored('Your paccoin.conf does not contain a \'masternodeprivkey\' entry, please set it up before opening your wallet', 'red'))

        # Rewrite config
        with open(config.paccoin_conf, 'w') as fp:
            for k, v in tokens.items():
                fp.write('{}={}\n'.format(k, v))

        print(colored('Done checking\n', 'green'))
        print(colored('Please open your wallet (paccoind on linux) and wait for it completely load/open before continuing', attrs=['bold']))
        ...
        print(colored('If it says it can not connect, check your paccoin.conf settings', 'yellow'))
...
if __name__ == '__main__':
    ...
    print(colored('Using paccoin.conf: {}'.format(config.paccoin_conf), 'green'))
    ...
    if option == 1: 
        ...
        data_folder = os.path.dirname(config.paccoin_conf)
